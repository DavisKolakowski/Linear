...
namespace LinearConflictService.Logic
{
    ...
    {
        public static async Task<IEnumerable<SpotFileMapperModel>> ReadSpotsListAsync(string distServer)
        {
            var dirInfo = new DirectoryInfo(distServer);
            Regex reg = new Regex(@"(<?filename>\d{6})\.txt");
            string file = (from f in dirInfo.GetFiles().Where(path => reg.IsMatch(path.ToString())) orderby f.LastWriteTime descending select f).First().FullName;
